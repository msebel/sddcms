...
if (isset($_POST['cmdSave'])) {
	$sLink = mediaConst::FILEPATH;
	$sLink.= $_POST['sFile'];
	$sLink = str_replace('{PAGE_ID}', page::id(), $sLink);
	$sLink = str_replace('{ELE_ID}', $nElement, $sLink);
}
?>
...
<html>
	<head>
		...
		<script src="/scripts/system/additional.js" type="text/javascript"></script>
	</head>
	<body scroll="no" style="OVERFLOW: hidden">
	<form name="mmAction" action="fck_link.php" method="post">
				...
				<table cellspacing="0" cellpadding="0" width="100%" border="0" dir="ltr">
					<tr>
						...
						<td nowrap="nowrap" width="100%">
							...
							<input value="<?php echo $sLink; ?>" id="txtUrl" style="width:100%" type="text" onkeyup="OnUrlChange();" onchange="OnUrlChange();" /><br>
						</td>
					</tr>
					<tr>
						<td nowrap="nowrap">
						...
						<td nowrap="nowrap">&nbsp;</td>
						<td nowrap="nowrap" width="100%">
							<div style="display:none;" id="divLightboxTick">
								<input type="checkbox" value="1" id="chkLightboxTick"> <?php echo $Res->html(505,page::language()); ?>
							</div>
						</td>
					</tr>
					<tr>
						<td colspan="2">&nbsp;</td>
						<td><?php echo $Res->html(506,page::language()); ?><br><br>
              <input type="checkbox" value="1" id="useSeoLink" checked="checked"> Eigenen Link verwenden, sofern vorhanden<br>
							<select id="menupunkte" name="menupunkte" onChange="setMenuitemLink()" style="width:100%">
								<option value="">-----</option>
								<?php
								// Menu anzeigen
								$items = $Menu->getMenuObjects();
                foreach ($items as $item) {
                  $sLevel = '';
                  for($i = 0;$i < $item->Level;$i++) {
                    $sLevel .= '--';
                  }
                  echo '<option value="'.$item->ID.';'.$item->getLink().'">'.$sLevel.' '.htmlentities($item->Name).'</option>';
                }
								?>
							</select>
						</td>
