<?php
class calendarYearPdf extends fPdf {
	/**
	 ...
	 */
	public function setExportList(calendarPdfYearList &$List) {
	...
	public function generate() {
		...
		for ($m = 0;$m < 12;$m++) {
			$this->ExportList->Day = $this->ExportList->Months[$m];
			$this->ExportList->Month = ($m+1);
			// Anzahl Tage definieren für Berechnung
			$this->ExportList->Days = date(
				't',
				mktime(
					0,
					0,
					0,
					$this->ExportList->Month,
					1,
					$this->ExportList->Year
				)
			);
			if ($m > 0) {
				$this->printTitle();
			...
			// Tage ausgeben
			for ($d = 0;$d < $this->ExportList->Days; $d++) {
				if ($this->checkNext($d)) {
					$this->printTitle();
				} 
				$this->printDays($d);
	...
	private function initializePDF() {
		// Ersten Monat auswählen
		$this->ExportList->Month = 1;
