...
class moduleTeaserCalendar extends commonModule {
	...
	public function saveConfig($nTapID,&$Config) {
		...
		$Config['eventCount']['Value'] = numericOps::validateNumber($_POST['eventCount'],0,50);
		...
		session_write_close();
		redirect('location: /admin/calendarteaser/index.php?id='.page::menuID().'&element='.$nTapID);
	}
	...
	public function initConfig($nTapID,&$Config) {
		...
		if (!teaserConfig::hasConfig($nTapID,$this->Conn,4)) {
			...
			teaserConfig::setConfig($nTapID,$this->Conn,3,pageConfig::TYPE_NUMERIC,'eventCount',$Config);
		} else {
			// Konfiguration laden
