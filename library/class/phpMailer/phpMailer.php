...
class phpMailer {
  ...
  function Send() {
	...
	if (!DEBUG) {
		...
		$result = $this->DebugMail($header, $body);
	}
    return $result;
  }

  /**
   * Gibt ein Mail in den Debug Output
   * @param string $header Kopfdaten des Mails
   * @param string $body Inhalt des Mails
   */
  function DebugMail($header,$body) {
	  $sPath = BP.'/doc/temp/'.date('d-m-Y-',time()).microtime(true).'.txt';
	  // Datei beschreiben
	  file_put_contents($sPath, $header.$body);
	  return(true);
  }
